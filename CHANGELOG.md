...
propagates it to all 17 sites. **`VERSION` rather than `git describe`**, which the issue
...
New `version-guard` CI job (blocking, toolchain-free) asserts all 17 agree, and on a tag
...
Separately, re-indenting `build.sbt` so its anchored pattern stopped matching failed
**loudly** (`pattern matched 0 times, expected exactly 1`) rather than silently passing —
a guard that degrades into a no-op when a manifest is reformatted would reintroduce the
