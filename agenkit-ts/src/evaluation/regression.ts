...
import { EvaluationResult } from './core';
...
export interface Regression {
  /** Name of metric that regressed */
  metricName: string;
  /** Baseline value for comparison */
  baselineValue: number;
  /** Current (degraded) value */
  currentValue: number;
  /** Degradation as percentage (positive = worse) */
  degradationPercent: number;
  /** Severity level of regression */
  severity: Severity;
 ...
 */
export function isRegression(regression: Regression): boolean {
 ...
 */
export function regressionToDict(regression: Regression): Record<string, unknown> {
  return {
...
export interface RegressionDetectorConfig {
  /** Acceptable degradation thresholds per metric (default 10%) */
  thresholds?: Record<string, number>;
  /** Baseline evaluation result to compare against */
...
export class RegressionDetector {
  ...
  private baseline: EvaluationResult | null = null;
   ...
   */
  setBaseline(result: EvaluationResult): void {
   ...
   */
  detect(result: EvaluationResult, storeHistory: boolean = true): Regression[] {
    if (storeHistory) {
    ...
    if (result.contextLength !== undefined && this.baseline.contextLength !== undefined) {
      const reg = this.checkMetric(
        'context_length',
        this.baseline.contextLength,
    ...
    if (result.compressionRatio !== undefined && this.baseline.compressionRatio !== undefined) {
      const reg = this.checkMetric(
        'compression_ratio',
        this.baseline.compressionRatio,
   ...
   */
  private calculateSeverity(degradation: number): Severity {
    if (degradation < 0.1) {
   ...
   */
  getTrend(metricName: string, window: number = 10): TrendStats | null {
   ...
   */
  getBaseline(): EvaluationResult | null {
