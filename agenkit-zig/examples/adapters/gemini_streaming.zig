...
///   zig build run-gemini-streaming

const std = @import("std");
const agenkit = @import("agenkit");
const GeminiLLM = agenkit.adapter.gemini.GeminiLLM;
...
pub fn main() !void {
    ...
    var llm_impl = try GeminiLLM.init(
        allocator,
        "", // Will read from GEMINI_API_KEY env var
    ...
    std.debug.print("=== Google Gemini Streaming Example ===\n\n", .{});
    std.debug.print("Asking Gemini to count to 10...\n\n", .{});
    ...
    std.debug.print("Asking Gemini to write a short story...\n\n", .{});
    std.debug.print("Response (streaming):\n", .{});
    std.debug.print("{s}\n", .{"-" ** 60});
