    ...
    def route(self, prompt: str, context: Optional[str] = None) -> RoutingDecision:
        ...
        if self.budget_limit and self.cost_tracker.total_cost >= self.budget_limit:
            return RoutingDecision(
                ...
                estimated_cost=MODELS["llama-2-70b"].cost_per_1k_tokens * classification.estimated_tokens / 1000
            )
        ...
        if classification.level == ComplexityLevel.CRITICAL:
            return RoutingDecision(
                ...
                estimated_cost=MODELS["gpt-4"].cost_per_1k_tokens * classification.estimated_tokens / 1000
            )
            ...
            return RoutingDecision(
                ...
                estimated_cost=MODELS["claude-2"].cost_per_1k_tokens * classification.estimated_tokens / 1000
            )
            ...
            return RoutingDecision(
                ...
                estimated_cost=MODELS["gpt-3.5-turbo"].cost_per_1k_tokens * classification.estimated_tokens / 1000
            )

    async def execute(self, prompt: str, context: Optional[str] = None, verbose: bool = True) -> str:
        ...
        if verbose:
            ...
            print(f"{'='*70}")
            print(f"Complexity: {classification.level.value} (confidence: {classification.confidence:.2f})")
        ...
        try:
            ...
            self.cost_tracker.record(RequestMetrics(
                prompt=prompt,
            ...
            if verbose:
                print(f"✓ Success with {decision.primary_model} ({latency:.0f}ms, ${actual_cost:.6f})")
